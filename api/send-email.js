      ...
      await resend.emails.send({
        ...
        html: `
          <div style="font-family:'DM Sans',sans-serif;background:#0d0d0d;padding:40px;max-width:600px;margin:0 auto;border-radius:16px;">
            ...
            <p style="color:#888;font-size:15px;margin:0 0 28px;">Great work this week! ${data.coachName} has reviewed your check-in and sent you personalised feedback.</p>
            ${data.coachNote ? `
            <div style="background:#161616;border:1px solid #2a2a2a;border-radius:12px;padding:24px;margin-bottom:28px;">
              <div style="color:#f5a623;font-size:11px;text-transform:uppercase;letter-spacing:1px;margin-bottom:10px;">Note from ${data.coachName}</div>
              <div style="color:#ccc;font-size:15px;line-height:1.7;">${data.coachNote}</div>
            </div>` : ''}
            <a href="${data.feedbackUrl}" style="display:inline-block;background:#f5a623;color:#000;text-decoration:none;padding:14px 28px;border-radius:10px;font-weight:700;font-size:15px;margin-bottom:28px;">View Full Feedback →</a>
            <p style="color:#444;font-size:12px;margin-top:8px;">Sent by ${data.coachName} via Akeema · <a href="mailto:${data.coachEmail}" style="color:#f5a623;">Reply to your coach</a></p>
          </div>
        `
      });
    }
