...
export default function App() {
            ...
            <>
              ...
              <div style={{ background: "#161616", border: "1px solid #2a2a2a", borderRadius: 12, padding: 28, marginBottom: 16 }}>
                ...
                <div style={{ marginBottom: 20 }}>
                  ...
                  <div className="adherence-5" style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 8 }}>
      ...
      <div style={{ background: "#0d0d0d", minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "sans-serif" }}>
        <style>{`
          ...
          * { box-sizing: border-box; }
          ...
          [class*="cl-badge"], [class*="Badge"] { background: #f5a623 !important; color: #000 !important; font-weight: 600 !important; font-size: 10px !important; }
        ...
        <SignIn appearance={{ variables: { colorPrimary: "#f5a623", colorBackground: "#161616", colorText: "#ffffff", colorInputBackground: "#1e1e1e", colorInputText: "#ffffff", colorTextSecondary: "#999999", colorNeutral: "#ffffff" }, elements: { card: { border: "1px solid #2a2a2a", boxShadow: "none" }, socialButtonsBlockButton: { border: "1px solid #333", background: "#1e1e1e", color: "#fff" }, socialButtonsBlockButtonText: { color: "#fff" }, badge: { background: "#f5a623", color: "#000", fontWeight: 700 }, formFieldLabel: { color: "#999" }, formFieldLabelRow: { color: "#999" }, formFieldHintText: { color: "#f5a623", fontWeight: 600 }, dividerLine: { background: "#2a2a2a" }, dividerText: { color: "#555" }, footerActionLink: { color: "#f5a623" }, footerActionText: { color: "#aaaaaa" }, otpCodeFieldInput: { background: "#1e1e1e", border: "1px solid #444", color: "#ffffff", borderRadius: "8px" }, identityPreviewText: { color: "#ffffff" }, identityPreviewEditButton: { color: "#f5a623" }, formHeaderTitle: { color: "#ffffff" }, formHeaderSubtitle: { color: "#999" }, alertText: { color: "#ffffff" }, alert: { background: "#1e1e1e", border: "1px solid #333" } } }} />
      </div>
    );
  }
    ...
    return (
      <div style={{ background: bg, minHeight: "100vh", padding: "40px 20px", fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          ...
          <div className="review-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 20 }}>
            {[
  ...
  if (view === "progress" && selectedClient) {
    ...
    function MiniChart({ data, labels, color, height = 80 }) {
      ...
      const pts = data.map((v, i) => `${(i/(data.length-1))*100},${height-((v-min)/range)*(height-8)-4}`).join(" ");
      return (
        <div>
          <svg viewBox={`0 0 100 ${height}`} style={{ width: "100%", height, overflow: "visible" }}>
            ...
            {data.map((v,i) => <circle key={i} cx={(i/(data.length-1))*100} cy={height-((v-min)/range)*(height-8)-4} r="3" fill={color}/>)}
          </svg>
    ...
    return (
      <div style={{ background: bg, minHeight: "100vh", fontFamily: "'DM Sans', sans-serif", overflowX: "hidden" }}>
        ...
        <div style={{ maxWidth: 860, margin: "0 auto", padding: "clamp(24px,4vw,40px) clamp(16px,4vw,40px)", animation: "fadeUp .4s ease" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 32 }}>
          ...
          {approved.length === 0 ? (
            <div style={{ ...card, padding: 60, textAlign: "center" }}>
              ...
              <p style={{ color: "#444", fontSize: 13, margin: 0 }}>Progress charts appear once you approve check-ins for this client.</p>
            </div>
          ) : (
            <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 20 }}>
                {[
                  { title: "Weight", unit: " lbs", data: weights, color: accent },
                  { title: "Adherence", unit: "%", data: adherenceData, color: "#3b82f6" },
                  { title: "Sleep Quality", unit: "/10", data: sleepData, color: "#818cf8" },
                  { title: "Energy", unit: "/10", data: energyData, color: green },
                  { title: "Stress", unit: "/10", data: stressData, color: red },
                ].map(chart => {
                  const latest = chart.data[chart.data.length-1];
                  const change = chart.data.length >= 2 ? (latest - chart.data[chart.data.length-2]).toFixed(1) : null;
                    ...
                    <div key={chart.title} style={{ ...card, padding: 20 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 16 }}>
                        ...
                        <div style={{ display: "flex", alignItems: "baseline", gap: 6 }}>
                          <span style={{ color: "#fff", fontSize: 20, fontWeight: 700 }}>{latest}{chart.unit}</span>
      ...
      <div style={{ background: "#0d0d0d", minHeight: "100vh", fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <style>{`
          ...
          @media (max-width: 768px) {
            .settings-mobile-nav { display: flex !important; overflow-x: auto; border-bottom: 1px solid #1e1e1e; padding: 0 16px; gap: 4px; -webkit-overflow-scrolling: touch; scrollbar-width: none; }
            .settings-mobile-nav::-webkit-scrollbar { display: none; }
            .settings-desktop-sidebar { display: none !important; }
            .settings-body { flex-direction: column !important; }
            .settings-content-area { padding: 24px 16px !important; max-width: 100% !important; }
          }
        `}</style>
        ...
        <div className="settings-body" style={{ display: "flex", minHeight: "calc(100vh - 73px)" }}>
          ...
          {/* Content */}
          <div className="settings-content-area" style={{ padding: "40px", maxWidth: 640, flex: 1, animation: "fadeUp .3s ease" }}>
                        ...
                        <div>
                          ...
                          <div style={{ color: "#444", fontSize: 11, marginTop: 2 }}>
                            {client.last_reminded_at
                              ? (() => { const d = new Date(new Date(client.last_reminded_at).toLocaleString("en-US", { timeZone: "America/New_York" })); return `Last reminded: ${d.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })} at ${d.toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit", hour12: true })}`; })()
  ...
  return (
    <div style={{ background: bg, minHeight: "100vh", fontFamily: "'DM Sans', sans-serif", overflowX: "hidden", width: "100%" }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        @media (max-width: 768px) {
          ...
          .trial-banner { flex-direction: column !important; align-items: flex-start !important; }
          .trial-banner button { width: 100% !important; }
          .dash-padding { padding: 20px 16px !important; }
          .dash-header-pad { padding: 14px 16px !important; }
          .dash-title { font-size: 24px !important; }
          .review-grid { grid-template-columns: repeat(2, 1fr) !important; }
          .adherence-5 { grid-template-columns: repeat(3, 1fr) !important; }
        }
      ...
      <div className="dash-padding" style={{ padding: "40px", width: "100%", overflowX: "hidden" }}>
        {loading ? (
